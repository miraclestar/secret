...
public class SecretService {
    ...
    public static String reply(HttpServletRequest request) {
        ...
        if (null != postStr && !postStr.isEmpty()) {
            ...
            if (msgType != null) {
                ret = BirdSing.singAsong("秘密必须是图片，你发一个图片，我回一个图片，看谁的更有料～", fromUsername, toUsername, time);
                ...
                if (msgType.equals("image")) {
                    ...
                    if (content.equals("lhy")) {
                        ret = BirdSing.tellSecret(fromUsername, toUsername, time);
                    } else {
                        ret = BirdSing.singAsong("只有发送图片才可以哟～", fromUsername, toUsername, time);
                    }
                ...
                // ret =
                // BirdSing.singAsongVideo("s-K03TfkqeXJ2I3OICEAFQn5ZmqT-qdrzBoDdPa271TZXlS9DjGLlkT_vRDx4J9z",
                // fromUsername, toUsername, time, "video");
                ...
                // ret = BirdSing.singAsongVoice(reMediaId, fromUsername, toUsername, time,
                // "voice");
                //
                // } else {
    ...
    private static String textReply(Element root, String fromUsername, String toUsername, String time) {
        ...
        /**
         * if (content.startsWith("WD")) { // 查看我的秘密 ret = BirdSing.showSecret(fromUsername,
         * toUsername, time); } else if (content.startsWith("CK") || content.startsWith("ck") ||
         ...
         * (content.substring(2).equals("")) { ret = BirdSing.singAsong("请加上你的留言，留言请以'LY'开头，谢谢",
         * fromUsername, toUsername, time); } else { DBDog.saveLiuyan(fromUsername,
         * content.substring(2)); ret = BirdSing.singAsong("留言成功,你可以输入CK来查看所有人的留言.", fromUsername,
         * toUsername, time); } } else if (content.substring(0, 1).equalsIgnoreCase("MM")) { // 文字秘密
         * if (content.substring(2).equals("")) { ret = BirdSing.singAsong("请加上你的秘密，秘密以'MM'开头，谢谢",
         * fromUsername, toUsername, time); } else { DBDog.saveWZSecret(fromUsername,
         * content.substring(2)); ret = BirdSing.singAsong("作为交换，告诉你这个秘密：" +
         * BirdSing.getRandomWZMM(fromUsername), fromUsername, toUsername, time); } } else if
         * (content.substring(0, 1).equalsIgnoreCase("PL")) { // 评论 DBDog.saveLiuyan(fromUsername,
         * content.substring(2)); ret = BirdSing.singAsong("pl成功,你可以输入CP来查看自己的评论.", fromUsername,
         * toUsername, time);
         * 
         * } else { // 非命令，提醒用户使用方法 ret =
         * BirdSing.singAsong("秘密请以'MM'开头，来交换别人的秘密；如要发留言板,请输入以'LY'开头的内容。谢谢", fromUsername,
         * toUsername, time); }
         **/
        return ret;
    }
